...
# Metadata may come from different locations depending on the lockfile, this struct should
# have data normalized across lockfiles.
def _new_package_info(id, name, dependencies, optional_dependencies, exclusively_dev, has_bin, optional, requires_build, version, friendly_version, resolution):
    return {
        "id": id,
        "name": name,
        "dependencies": dependencies,
        "optional_dependencies": optional_dependencies,
        "exclusively_dev": exclusively_dev,
        "has_bin": has_bin,
        "optional": optional,
...
def _convert_v5_packages(packages):
    ...
    for package_path, package_snapshot in packages.items():
        ...
        package_info = _new_package_info(
            ...
            exclusively_dev = package_snapshot.get("dev", False),
...
def _convert_v9_packages(packages, snapshots):
    ...
    for package_key, package_snapshot in snapshots.items():
        ...
        package_info = _new_package_info(
            ...
            exclusively_dev = None,  # NOTE: pnpm v9+ no longer marks packages as dev-only
